...
-- 1 - Dado nome de uma cidade, deseja-se saber:
    ...
    select * from Cidade as c inner join Restaurante as r on c.ID_Cidade = r.Cidade_ID inner join Casa_de_Show as cs on cs.Especialidade_Rest = "Fast Food" and cs.Dia_de_Fechamento != "Sabado" and r.ID_Restaurante = cs.Restaurante_ID
	
    -- 2.2 Quais os hotéis que possuem restaurantes daquela especialidade, abertos nas datas indicadas
    
    select * from Hotel as h inner join Casa_de_Show as cs on h.Restaurante_ID = cs.Restaurante_ID and cs.Especialidade_Rest = "Churrasco" and cs.Dia_de_Fechamento != "Terça-Feira"
    ...
    select * from Casa_de_Show as cs inner join Restaurante as r on cs.Restaurante_ID = r.ID_Restaurante and cs.Dia_de_Fechamento = "Segunda-Feira" and  cs.Especialidade_Rest != "Fast Food"
